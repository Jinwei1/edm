...
@click.option('--cond',          help='Train Any-conditional model', metavar='BOOL',              type=bool, default=False, show_default=True)
...
@click.option('--ema',           help='EMA half-life', metavar='MIMG',                              type=click.FloatRange(min=0), default=0.05, show_default=True)
...
@click.option('--dump',          help='How often to dump state', metavar='TICKS',                   type=click.IntRange(min=1), default=1, show_default=True)
...
def main(**kwargs):
    ...
    c.resize_resolution = opts.resize_res
    c.gamma_correction = opts.gamma_correction
    ...
    if opts.arch == 'ddpmpp':
        c.network_kwargs.update(model_type='JinSongUNet', embedding_type='positional', encoder_type='standard', decoder_type='standard')
        ...
        c.network_kwargs.update(model_type='JinDhariwalUNet', model_channels=192, channel_mult=[1,2,3,4])
    ...
    dist.print0(f'Any-conditional:       {c.dataset_kwargs.use_labels}')
    dist.print0(f'Network architecture:    {opts.arch}')
    dist.print0(f'Preconditioning & loss:  {opts.precond}')
    dist.print0(f'Number of GPUs:          {dist.get_world_size()}')
    dist.print0(f'Batch size:              {c.batch_size}')
    dist.print0(f'Mixed-precision:         {c.network_kwargs.use_fp16}')
    dist.print0(f'resize_res:              {c.resize_resolution}')
    dist.print0(f'gamma_correction:        {c.gamma_correction}')
    dist.print0()
