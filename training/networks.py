...
@persistence.persistent_class
class JinSongUNet(torch.nn.Module):
    def __init__(self,
...
class JinDhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        # self.conv_label2 = Conv2d(in_channels=label_dim, out_channels=label_dim, kernel=3, **init) if label_dim else None
    ...
    def forward(self, x, noise_labels, any_labels, augment_labels=None):
        ...
        if self.conv_label1 is not None:
            tmp = any_labels
            if self.training and self.label_dropout:
                tmp = tmp * (torch.rand([x.shape[0], 1], device=x.device) >= self.label_dropout).to(x.dtype)
